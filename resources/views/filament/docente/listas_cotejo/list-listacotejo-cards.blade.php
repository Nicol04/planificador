    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" wire:loading.class="opacity-50">
            @forelse($this->getFilteredListas() as $lista)
                <div
                    class="bg-white rounded-xl shadow-sm p-5 dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                    @php
                        ...
                        if (is_string($nivelesRaw)) {
                            ...
                            $niveles = is_array($decodedNiv)
                                ? $decodedNiv
                                : array_filter(array_map('trim', explode(',', $nivelesRaw)));
                        ...
                        <div class="flex flex-wrap gap-2 mb-3">
                            @foreach ($niveles as $n)
                                @php
                                    ...
                                    // verde
                                    elseif (strpos($lc, 'en proceso') !== false || strpos($lc, 'enproceso') !== false) {
                                        $dotColor = '#f59e0b';
                                    }
                                    // amarillo
                                    elseif (strpos($lc, 'no logrado') !== false || strpos($lc, 'nologrado') !== false) {
                                        $dotColor = '#ef4444';
                                    }
                                    // rojo
                                    else {
                                ...
                                @endphp
                                <span
                                    class="inline-flex items-center px-2 py-0.5 bg-white dark:bg-gray-800 rounded-full border border-gray-200 dark:border-gray-700 text-xs">
                                    <span
                                        style="display:inline-block;width:8px;height:8px;border-radius:9999px;margin-right:8px;background-color: {{ $dotColor }};border:1px solid rgba(0,0,0,0.08);"></span>
                                    {{ $label }}
                                </span>
                            @endforeach
                        </div>
                        ...
                        @if (!empty($propositos))
                            <div class="mt-2">
                                ...
                                @foreach ($propositos as $p)
                                    ...
                                    @if ($compNombre)
                                        <div
                                            class="mt-2 p-3 bg-gray-50 dark:bg-gray-700 rounded-md border border-gray-100 dark:border-gray-600 text-sm font-medium">
                                            {{ $compNombre }}
                                        </div>
                                    @elseif($compId)
                                        <div
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        {{-- Mantener solo descarga sin plantilla (horizontal por defecto en raw) --}}
                        <x-filament::dropdown>
                            <x-slot name="trigger">
                            ...
                            <x-filament::dropdown.list>
                                <x-filament::dropdown.list.item
                                    onclick="(function(){ const u='{{ route('listas-cotejo.previsualizar', ['id' => $lista->id]) }}?orientacion=horizontal&raw=1'; window.open(u,'_blank'); })()"
                                    icon="heroicon-o-arrow-down-tray">
                                    💾 Descargar Word Horizontal (sin plantilla)
                                </x-filament::dropdown.list.item>
                            </x-filament::dropdown.list>
                        </x-filament::dropdown>
                    </div>
            ...
            @empty
                <div class="col-span-full">
                    <div
                        ...
                        <h3 class="text-xl font-semibold text-gray-900 dark:text-white mb-2">No hay listas de cotejo
                        </h3>
                        ...
                        <a href="{{ route('filament.docente.resources.lista-cotejos.create') }}"
        ...
        <div style="background: white; border-radius:10px; padding:24px; max-width:480px; text-align:center;">
            <div style="display:flex; gap:12px; justify-content:center; margin-bottom:18px;">
                <button onclick="abrirVistaPreviaListasConOrientacion('horizontal')"
                    style="background:#28a745; color:white; padding:10px 16px; border:none; border-radius:6px; cursor:pointer;">Horizontal</button>
            </div>
            <div style="display:flex; gap:10px; justify-content:center; margin-bottom:8px;">
                <button onclick="imprimirDesdeModalListas()"
                    style="background:#17a2b8; color:white; padding:8px 12px; border:none; border-radius:6px; cursor:pointer;">🖨️
                    Imprimir</button>
