...
<body class="bg-light">
    ...
    <div class="floating-buttons no-print">
        <button class="btn btn-success btn-floating" onclick="descargarDocumentoVertical()">
    ...
    <div class="document-preview">
        ...
        <div class="p-4">
                        ...
                        @foreach ($cursosInfo as $cursoIndex => $cursoInfo)
                            ...
                            @if ($totalCompetencias > 0)
                                ...
                                @foreach ($competencias as $compIndex => $competenciaInfo)
                                    <tr
                                        style="border-top: {{ $compIndex === 0 ? '3px solid #0066cc' : '1px solid #dee2e6' }};">
                                            ...
                                            @if ($competenciaInfo['capacidades']->count() > 0)
                                                <div class="ms-3" style="margin-top: 8px;">
                                                    <strong class="text-success"
                                                        style="font-size: 9px;">Capacidades:</strong>
                                ...
                                @if (!$loop->last)
                                    <tr>
                                        <td colspan="6"
                                            style="height: 4px; background: linear-gradient(45deg, #0066cc, #004499); border: none; padding: 0;">
                                        </td>
                                    </tr>
                                @endif
                            @else
            ...
            @if (!empty($cronograma))
                <!-- Ajuste: Eliminar cualquier wrapper que agregue scroll y asegúrate de que la tabla ocupe el 100% -->
                <div class="cronograma-table-wrapper">
                    <table class="table table-bordered w-100" style="font-size:12px; min-width: 700px;">
                        <thead>
                            <tr>
                                <th>Semana</th>
                                <th>Día</th>
    ...
    <script>
        // Función para descargar Word en formato vertical
        function descargarDocumentoVertical() {
            window.location.href = `/unidades/{{ $unidad->id }}/previsualizar?orientacion=vertical&descargar=1&raw=1`;
        }
